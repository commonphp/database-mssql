...
final class MssqlStatementBinder
{
    /**
    ...
    public function bind(PDOStatement $statement, array $parameters, string $query = ''): void
    {
        ...
        foreach ($parameters as $key => $value) {
            ...
            if (!$statement->bindValue($parameter, $this->normalizeValue($value), $this->parameterType($value))) {
                throw MssqlQueryException::forBinding($parameter, $query);
            }
        }
    }
    ...
    private function namedParameter(string $key): string
    {
        ...
        if ($key === '' || $key === ':' || $key === '@') {
            throw MssqlQueryException::forInvalidParameter($key, 'named parameters cannot be empty.');
        }
    ...
    {
        if ($key < 0) {
            throw MssqlQueryException::forInvalidParameter($key, 'positional parameters cannot be negative.');
        }
